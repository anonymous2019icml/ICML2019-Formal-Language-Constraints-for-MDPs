...
def build_dqn(env_name, contract, architecture, contract_mode, steps, nb_actions, emb, enforce_contract, log_prefix):
...
def train(env_name, contract, architecture, contract_mode, steps, train_seed, emb, enforce_contract, doom_scenario):
    ...
    dqn = build_dqn(env_name, contract, architecture, contract_mode, steps, nb_actions, emb, enforce_contract, log_prefix)

    callbacks = [ContractLogger(log_prefix + '_callback.csv')]
    dqn.fit(
        env,
        nb_steps=steps,
        log_interval=10000,
...
def test(env_name, contract, architecture, contract_mode, steps, train_seed, test_seed, emb, enforce_contract, doom_scenario):
    ...
    weights_filename = os.path.join(args.weights_root, filename_prefix_fn(env_name, contract, architecture, contract_mode, steps, train_seed, None)) + '_weights.h5f'
...
if __name__ == '__main__':
    if args.task == 'train':
        ...
        test(args.env_name, args.contract, args.architecture, args.contract_mode, args.steps, args.train_seed, args.test_seed, args.emb, args.enforce_contract, args.doom_scenario)
    else:
